...
cartRouter.put('/:cid/products/:pid',async (req,res) => {
  ...
  try {
    const cart =  await cartModel.findById(cid);
    if (cart){
      const indexToUpdate = cart.products.findIndex(producto => producto.id_prod._id.toString() === pid);
      cart.products[indexToUpdate].quantity = quantity;
      cart.save();
      res.status(200).send({ resultado: 'OK', carrito: cart.products[indexToUpdate] });
    }else{
      res.status(404).send({ resultado: 'Carrito no encontrado', carrito: cart });
    }
  }
  ...
  try {
    ...
    if (cart){
      products.forEach(nuevoProducto => {
        const indexOfProduct = cart.products.findIndex(producto => producto.id_prod._id.toString() === nuevoProducto.id_prod);
        if(indexOfProduct !== -1){
